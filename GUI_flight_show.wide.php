...
if (0) {
	...
	echo "<img src='"..$CONF['protocol']."://maps.google.com/staticmap?center=$cLat,$cLon&span=$sLat,$sLon&size=512x512&key=$CONF_google_maps_api_key&sensor=false&format=jpg&maptype=hybrid'><HR>";
	//exit;
}
...
if ( $CONF_google_maps_track==1 && $PREFS->googleMaps ) {
	...
	if ( $CONF_google_maps_api_key  ) {
		 $googleMap="<div id='gmaps_div' style='display:block; width:945px; height:610px;'><iframe id='gmaps_iframe' align='left'
		  SRC='"..$CONF['protocol']."://".$_SERVER['SERVER_NAME'].getRelMainDir()."EXT_google_maps_track.php?id=".
		$flight->flightID."' ".
