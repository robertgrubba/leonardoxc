...
echo "<button id='embed' style=' margin-top: -30px; float: right; background-color: #E1E6F3; border: 1px solid #d3cfe4; color: #006699;'>&#60;\&#62;</button>";
if($userID && $pilotID == $userID){
	echo "<div id='embed' class='embed' style='display:none; float:right; margin-top:5px; margin-bottom:5px; background-color: #E1E6F3;border-radius:5px 5px 5px 5px; padding:5px;'>W celu osadzenia stopki na blogu, stronie lub forum możesz zastosować HTML lub tzw BBCODE <p><img src='".$CONF['links']['baseURL']."/data/pilots/$pilotID/stopka_polish_niebieska.png'><ul><li><strong>BBCODE: </strong>[url=".getLeonardoLink(array('op'=>'pilot_profile_stats','pilotID'=>$serverIDview.'_'.$pilotIDview,'year'=>'0','country'=>''))."][img]".$CONF['links']['baseURL']."/data/pilots/$pilotID/stopka_polish_niebieska.png[/img][/url]</li><li><strong>HTML: </strong>&lt;a href='".getLeonardoLink(array('op'=>'pilot_profile_stats','pilotID'=>$serverIDview.'_'.$pilotIDview,'year'=>'0','country'=>''))."'&gt;&lt;img src='".$CONF['links']['baseURL']."/data/pilots/$pilotID/stopka_polish_niebieska.png' alt='Statystyki pilota na LeonardoXC'&gt;&lt;/a&gt;</li></ul></p>"
	...
	echo "<div id='embed' class='embed' style='display:none; float:right; margin-top:5px; margin-bottom:5px; background-color: #E1E6F3;border-radius:5px 5px 5px 5px; padding:5px;'><p>Zalogowani użytkownicy na stronie swoich statystyk zobaczą kod (HTML i BBCODE), który można wykorzystać do osadzenia grafiki ze statystykami na blogu, stronie internetowej lub forum np. w stopce.</p></div>";
}
?>
...
//	}	
?>
    <td colspan="5" valign="top"><img id="pilotProfileStats" src='<? echo getPilotStatsRelFilename($pilotIDview,'1'.$suffixHash); ?>' border=0></td>
  </tr>
  </table>
 ...
 <table  class=main_text  width="100%" border="0" cellpadding="3" cellspacing="3">
  <tr>
    <td colspan="5" valign="top"><img id="pilotProfileStats" src='<? echo getPilotStatsRelFilename($pilotIDview,'2'.$suffixHash); ?>' border=0></td>
  </tr>
</table>
