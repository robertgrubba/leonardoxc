...
//************************************************************************
	...
	for($k=0;$k<count($takeoffs);$k++) {
		$sel=($takeoffsID[$k]==$FILTER_takeoff1_select)?"selected":"";
		 $TAKEOFF_OPTION_LIST.="<option value='".$takeoffsID[$k]."' $sel>".$takeoffs[$k]." ".file_get_contents($CONF['weatherapi'].'/isflyabletoday/1/'.getWaypointIntName($takeoffsID[$k]))." ".showWaypointDesciptionIcon($takeoffsID[$k])." </option>\n";
	}
